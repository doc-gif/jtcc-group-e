...
/** 案内役「ピピ」の姿。パズルのピースに王冠をのせたオリジナルキャラクター。飾りなので読み上げない。 */
export function PipiFigure({ className = 'pipi-body' }: { className?: string }) {
  const gradient = `pipi-rg-${useId().replace(/[^a-zA-Z0-9_-]/g, '')}`
  return (
    <svg viewBox="0 0 80 84" aria-hidden="true" focusable="false" className={className}>
      <defs><linearGradient id={gradient} x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#FBE3D9" /><stop offset=".5" stopColor="#EDB3A3" /><stop offset="1" stopColor="#D98C86" /></linearGradient></defs>
      ...
      <path d="M24 18l4-10 6 7 6-11 6 11 6-7 4 10z" fill={`url(#${gradient})`} stroke="#fff" strokeWidth="1.5" strokeLinejoin="round" />
