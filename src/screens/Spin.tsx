...
import { DEMO_FRIENDS, shouldShowKakutei, spin, spinCheck, type SpinOutcome } from '../domain/game'
...
type Phase = 'ready' | 'turning' | 'dropping' | 'opening' | 'revealed' | 'board'
...
function SpinStage({ gacha, mode }: { gacha: Gacha; mode: Mode }) {
  ...
  const advance = (delta: number) => {
    if (delta <= 0 || (phase !== 'ready' && phase !== 'turning')) return
    if (!begin()) return
    setAcc((value) => Math.min(value + delta, FULL))
  }

  const oneTurn = () => {
    if (phase !== 'ready' && phase !== 'turning') return
    if (!begin()) return
  ...
  // 友達（デモ）の進み具合
  const spinning = phase !== 'ready' && companions.length > 0
  useEffect(() => {
  ...
  const again = () => {
    ...
    setWon(null); setAcc(0); setPhase('ready'); setBanner(null); setChip(null); setBurst(null); setBubbles({})
  ...
  const status = (index: number) => phase === 'ready' ? '準備OK' : progress[index] >= 1 ? '出た！' : `${Math.floor(progress[index] * TURNS)} / ${TURNS} 回転`

  return (
    <div className={`screen spin-screen${kakutei ? ' is-kakutei' : ''}`}>
      <LuxBackdrop level={turns} kakutei={kakutei} />
      <BackBar title={gacha.title} back={paths.gacha(gacha.id)} backLabel="詳細へ">
        <p className="backbar-sub">{mode === 'room' ? `ROOM・${companions.length + 1}人でいっしょに（友達はデモ）` : 'ひとりで回す'}</p>
      </BackBar>
      <main className="content spin-content">
        {mode === 'room' && (
          <ul className="spin-members" aria-label="ルームのメンバー">
            <MemberRing name={state.nickname} me progress={acc / FULL} bubble={bubbles.me} status={phase === 'ready' ? 'ハンドルを回してね' : turns >= TURNS ? '開封中…' : `${turns} / ${TURNS} 回転`} />
            {companions.map((name, index) => <MemberRing key={name} name={name} progress={progress[index]} bubble={bubbles[name]} status={status(index)} />)}
          </ul>
        )}
        <p className="lead spin-lead" aria-live="polite">{phase === 'ready' || phase === 'turning' ? `ハンドルを くるくる ${TURNS}回まわしてね（いま ${turns} / ${TURNS} 回転）` : 'ガチャが出たよ！'}</p>
        <p className="turn-dots" aria-hidden="true">{Array.from({ length: TURNS }, (_, i) => <span key={i} className={i < turns ? 'on' : ''}>{i < turns ? '♥' : '♡'}</span>)}</p>
        <div className="machine-wrap">
          <Machine price={gacha.price} angle={acc} className={machineClass} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} />
          {phase !== 'ready' && phase !== 'turning' && <span className={`drop-cap cap-${glow}`} aria-hidden="true" />}
        ...
        <button type="button" className="btn btn-main btn-block" onClick={oneTurn} disabled={phase !== 'ready' && phase !== 'turning'}>
          回しにくいときは、ここをタップ（1タップで1回転・{turns} / {TURNS}）
        </button>
        <p className="fine">指でハンドルを時計回りになぞって回します。回し始めた時点で {coinText(gacha.price)} コインを使い、結果が決まります。確定演出は、本当に目玉が出るときだけ出ます。</p>
        <MockNotice />
      </main>
      ...
      {won && (phase === 'opening' || phase === 'revealed') && (
        <OpenScene key={round} won={won} gacha={gacha} mode={mode} onReveal={onReveal} onTap={() => ring(CHIMES.tap, 30)}
...
interface OpenProps { won: Won; gacha: Gacha; mode: Mode; revealed: boolean; onReveal: () => void; onTap: () => void; onBoard: () => void; onAgain: () => void; canAgain: boolean }

function OpenScene({ won, gacha, mode, revealed, onReveal, onTap, onBoard, onAgain, canAgain }: OpenProps) {
  ...
  return (
    <div className={`open-scene glow-${won.prize.glow}${revealed ? ' is-revealed' : ''}`} role="dialog" aria-modal="true" aria-labelledby="open-title">
      {!revealed ? (
        <>
          <h2 id="open-title" className="open-title">カプセルが出ました</h2>
          <button ref={ref} type="button" className={`big-capsule step-${Math.min(taps, 2)}`} onClick={tap} aria-label={`カプセルをあける（あと${Math.max(need - taps, 0)}回タップ）`}>
            <span className="cap-top" /><span className="cap-bottom" /><span className="cap-band" />
          </button>
        ...
        <div className="prize-reveal">
          ...
          <h2 id="open-title" className="reveal-name">{won.prize.name}</h2>
    ...
    <div className="screen">
      ...
      <main className="content">
        {problem === 'insufficient-coins' ? (
          <>
            <p className="lead spin-lead">コインが足りません。1回 {coinText(gacha.price)} コインです。</p>
            <a className="btn btn-main" href={paths.me}>マイページでコインを追加</a>
          </>
        ) : (
          <>
