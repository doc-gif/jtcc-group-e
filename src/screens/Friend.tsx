...
import { findFriend, findPrizeById, friendsOf, hasReacted, SHELF_SIZE, sendReaction, shelfSlots, type DemoFriend } from '../domain/shelf'
...
function Unavailable({ friend, met }: { friend: DemoFriend; met: boolean }) {
  return (
    <div className="screen">
      <BackBar title="今は見られません" back={paths.together} backLabel="戻る" />
      <main className="content">
        ...
        <p className="lead unavailable-lead">{met
          ? '公開範囲や接続状態が変わるまで待ってください。'
          : `${friend.name}さんと、まだいっしょに回していません。いっしょに回すと、ここから棚を見られます（デモ）。`}</p>
        <p className="fine">あなたの棚や持ち物には影響しません。</p>
...
function useFriend(id: string) {
  ...
  return { friend, visible: friend !== undefined && met && friend.viewable, met }
}

/** 友だち（デモ）の棚（T09 267:8786）。自分の棚ではないことを見出しと各枠の文字で示す。 */
export function FriendShelf({ id }: { id: string }) {
  const { friend, visible, met } = useFriend(id)
  if (!friend) return <NotFound />
  if (!visible) return <Unavailable friend={friend} met={met} />
  ...
  return (
    <div className="screen">
      <BackBar title={`${friend.name}さんの棚`} back={paths.together} backLabel="戻る" />
      <main className="content">
        <p className="item-eyebrow">友だちの持ち物・あなたの棚ではありません</p>
        <div className="shelf-summary">
          <div className="shelf-summary-text">
            <p><b className="shelf-count">{friend.name}さんの棚・{entries.length} / {SHELF_SIZE} 枠</b></p>
            <p className="lead friend-lead">{friend.name}さんが飾っている品を見ています（デモ）。</p>
          </div>
        </div>
        <ShelfGrid slots={shelfSlots(entries)} label={`${friend.name}さんの棚（${SHELF_SIZE}枠・デモ）`} emptyTitle="まだ飾っていません" emptyNote="空き枠" hrefFor={(entry) => paths.friendItem(friend.id, entry.id)} />
...
export function FriendItem({ id, item }: { id: string; item: string }) {
  ...
  return (
    <div className="screen">
      <BackBar title={sent ? '気持ちを送った' : `${friend.name}さんの1点`} back={paths.friend(friend.id)} backLabel="戻る" />
      <main className="content">
