...
export async function resolvePreview({ github, owner, repo, runId }) {
  ...
  if (run.path !== '.github/workflows/ci.yml' || run.event !== 'pull_request' || run.status !== 'completed'
  ...
  if (!['Quality gate', 'UI/UX gate'].every((name) => jobs.some((job) => job.name === name && job.conclusion === 'success'))) return null
  ...
  return { pr: pr.number, sha: run.head_sha, runId, artifactId: builds[0].id, repository: `${owner}/${repo}` }
}
