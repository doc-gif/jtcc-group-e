...
const utc = (date) => new Date(date).toISOString().replace(/:\d\d\.\d{3}Z$/, '').replace('T', ' ')
...
export function formatReport(state) {
  ...
  section('PR のない最近のブランチ（48時間以内）', state.orphanBranches, (refs) => refs.length
    ...
    : ['- なし'])
  section('main にない migration', state.strayMigrations, (items) => items.length
...
async function collect({ git, fetchJson, now }) {
  const attempt = async (run) => { try { return await run() } catch (error) { return error instanceof Error ? error : new Error(String(error)) } }
  await attempt(() => git(['fetch', '--quiet', '--prune', 'origin']))
  const refs = await attempt(async () => parseRefs(await git(['for-each-ref', '--format=%(refname:short)%09%(objectname)%09%(committerdate:iso-strict)%09%(subject)', 'refs/remotes/origin'])))
  const main = await attempt(async () => {
  ...
  const mainMigrations = await attempt(async () => (await git(['ls-tree', '--name-only', 'origin/main', 'supabase/migrations/'])).split('\n').filter(Boolean).map((file) => file.split('/').pop()).sort())
  const strayMigrations = await attempt(async () => {
    ...
    return migrationsOutsideMain(mainMigrations, branchFiles)
  })
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (Object.values(state).some((value) => value instanceof Error)) process.exitCode = 1
}
