 ...
 */
export function planRelease({ tags, releases = [], sha, requested, isAncestor }) {
  ...
  if (released) {
    // タグだけでは完了の証拠にしない。Release とビルド ZIP がそろっていなければ、途中で失敗した公開なので成功扱いにしない。
    const release = releases.find((item) => item.tag_name === released.name)
    if (!release?.assets?.some((asset) => asset.name === 'web-app.zip')) throw new Error(`${released.name} はタグだけで Release かビルド ZIP がない（公開の途中で失敗）。新しく依頼せず、元の実行の失敗したジョブを再実行する（docs/DEPLOYMENT.md「失敗と復旧」）`)
    return { publish: false, version: released.name, reason: `この SHA は ${released.name} で公開済み（同じ SHA を二重に公開しない）` }
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  const releases = JSON.parse(run('gh', ['api', '--paginate', '--slurp', `repos/${process.env.GITHUB_REPOSITORY}/releases?per_page=100`])).flat()
  const plan = planRelease({ tags, releases, sha, requested: process.env.REQUESTED_VERSION?.trim() || '', isAncestor })
