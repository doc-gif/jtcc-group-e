...
- 作業者自身が `pnpm verify` と必要なレビューを完了し、PR を Ready に変更する。GitHub CLI なら `gh pr ready <番号>`。マージ許可を毎回聞き直さず、必須 CI と Bot のマージ完了まで追跡する。失敗は修正して再検証する。保護ルールを外さない。
...
`docs/PREVIEW.md` を読む。同一リポジトリの PR は Draft でも必須 CI の成功後に本番とは別のサイトへ自動公開される。確認用の公開に追加の指示は不要。PR の Bot コメントにある現在の SHA の URL を開き、表示と変更した操作を確認する。プレビューは本番公開ではない。本番の Release ワークフローを確認用の代わりに実行しない。
