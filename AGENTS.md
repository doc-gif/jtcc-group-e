...
- 指定 Figma ファイル `yeDF1BwhrxpXI57Daainle` をアプリのデザイン管理先とする。ラストピースの画面の正は [デザインマスター `267:8198`](https://www.figma.com/design/yeDF1BwhrxpXI57Daainle?node-id=267-8198)。55画面・状態と node の対応は [`docs/ADOPTED_DESIGN.md`](docs/ADOPTED_DESIGN.md)。緑アクセントの `252:2` は不採用なので元にしない。詳細・各部品のリンクは `docs/DESIGN.md`。
