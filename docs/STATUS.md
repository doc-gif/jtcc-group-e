...
| 候補 | 前提 | 次に確認する内容 |
| --- | --- | --- |
| （T15–T43 の番号と内容はこのリポジトリに記録がない） | T11・T14 | 担当者の作業計画で T15 以降の定義を確認してから選ぶ。T14 までの記録に残っている後続作業は、画面を `supabaseTransport` に接続する、Realtime を初期化して受信ポリシーを適用する、HTTP 経路・同時実行・40 端末で確かめる、デザインマスターを Web に実装する（T24 以降）、`design-system/` のトークンをピンク系に整理する。 |

T15 以降の番号付きの定義は、担当者の作業計画（リポジトリ外）にあります。後続の実装・検証・公開カードは前提完了後に選びます。通常依頼を優先し、引き継ぎは[担当者が明示的に希望した場合](HANDOFF.md)だけ開始します。
