...
最終更新: 2026-09-26 07:36 JST。今回の進捗ブランチは `docs/t11-adopted-design`、基点の `main` は [`835d73a`](https://github.com/doc-gif/jtcc-group-e/commit/835d73ae13c6dc4cee439711c9ca009391e4f3e9) です。`DONE` は各タスクの受け入れ条件を確認した意味で、アプリ全体の完成や本番公開を意味しません。Figma はライブ状態です。内部画像は公開リポジトリに置かず、採用版のSHA-256を記録します。
...
残り 31 件: `T13`–`T43`。現時点で前提がそろった候補は次のとおりです。ここに載せたことは自動割当ではありません。

| 候補 | 前提 | 次に確認する内容 |
| --- | --- | --- |
| T13 | T03 | SQL のスキーマ・認可・原子性を本番適用前にレビューする。 |

`T13` の前提はそろいました。後続の実装・検証・公開カードは前提完了後に選びます。通常依頼を優先し、引き継ぎは[担当者が明示的に希望した場合](HANDOFF.md)だけ開始します。
