...
今後の PR では、公開する画像の出典と見た目を確認し、[内部素材ライブラリ](ASSET_LIBRARY.md) の公式画像・商品写真を `public/`、ビルド成果物、公開プレビューに入れない。`VITE_` の値はブラウザから読めるため秘密を入れず、新しい環境変数や外部データを使うときは公開成果物を再検査する。F15 から、ビルドには `.env.production` の Supabase の URL と publishable キー（公開してよい値）が入り、確認用プレビューのルームも専用の Supabase につながる（[記録](MULTI_DEVICE.md)）。secret キーは入れない（`scripts/governance.test.mjs` で確認）。`.gitignore` と許可ディレクトリ検査だけを権利・秘密の最終判定にしない。
