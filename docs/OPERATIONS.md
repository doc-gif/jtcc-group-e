...
| `lock:supabase` / `lock:figma-master` | 共有資源を使用中。同時に1つだけ。期限15分（ロックを立てた時刻・最後の延長から）。延長はロック Issue へのコメント1行、1回15分まで、回数の上限は無い。延長が無ければ解放。終わったら Issue を閉じる | 作業者 |
...
4. 共有資源は `lock:*` の Issue を立ててから。開いているロックがあれば待ち、他の部分を先に進める。案は作業領域で作り、マスターへの反映時だけロックを立てる。持っている間は10分ごとに延長のコメントを書く。解放された資源を次に使うときは、新しいロックを立てる前に前の保持者に声をかけ15分待つ。
...
2. **止まっているものを見つける**
   ...
   - `lock:*` の Issue で15分の延長コメントが無い → ロック Issue に `[local-manager] 15分の延長がないためロックを解放します（規則）` と書いて閉じ、親 Issue にも知らせる。
