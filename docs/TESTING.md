...
- 「ホーム画面に追加」の案内（#57、マスター `448:14192`・`448:14267`）: `src/app/installGuide.test.tsx` で、タブで開いたときだけ出す・ホーム画面から開いた（`display-mode: standalone`・`navigator.standalone`）ときは出さない・「閉じる」でその端末に保存して二度と出さない・`beforeinstallprompt` があればボタンで追加の画面を出し、追加したら出さない・やめたら手順に戻る・`appinstalled` で消える・保存できない端末でも閉じられる、を確かめる。`e2e/standalone.spec.ts` で、5構成の実ブラウザで街ホームに出てルームには出ないこと、閉じたら再読み込みしても出ないこと、standalone を真似ると出ないことを確かめる。44px・文字200%・axe は `ux.spec.ts` の `town-home`（案内が出た状態）で確かめる。
