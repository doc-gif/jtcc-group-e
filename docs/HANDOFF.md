...
- **7 の一般公開アプリの部屋を作るとき**: 今の `lp_can_view_photos()` は開いているどの部屋の参加者でも true になる。誰でも入れる一般公開アプリの部屋を足す migration では、写真を限定公開アプリの部屋（`kind = 'pitch'`）だけに絞る（一般公開アプリに出す時期は #58 の子 Issue で決める。それまで出さない）。
...
| 写真 | 限定公開アプリの実物グッズの写真（F15-2） | **実写真のサンリオ商品画像だけ使える**（担当者の決定、#135 の用語の表。キャラクターのイラスト・ロゴ・アセットは不可）。出す時期と切り替えは #58 の子 Issue で決める（決まるまでオリジナルのイラスト。フラグで切り替え） |
...
3. アプリ: 一般公開アプリの入口（`#/demo` など）の画面、秒読み、ボットの表示、「もう一度参加」。写真は出す時期が決まるまで無効（フラグ）。
