...
**進み具合**: 1（全画面のメタ・manifest の検査）・2（ジェスチャーの抑制）・4（3 以外のテスト）・5（文書）はブランチ `claude/f16-standalone` の PR で入れた（STATUS.md の F16）。**残りは 3「ホーム画面に追加」の案内**（Figma のマスターが先。`lock:figma-master` の Issue が閉じてから）と 6 の担当者の実機確認。3 を作るときは、ホーム画面のアプリにはアドレスバーがなく、iPhone では保存データも Safari と別なので、招待リンク・ホスト用リンクからホーム画面のアプリで入る方法も一緒に決める（PRODUCT.md の未決事項）。ヘッドレスのブラウザは抑制なしでも合成したピンチで拡大しないため、ピンチの効きは実機でだけ確かめられる。
