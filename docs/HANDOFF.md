...
- アプリへの反映（色・リボン・丸い見出し・レイアウトだけ。**キャラクターの絵と © 表記の札はアプリに入れない**）は [#70](https://github.com/doc-gif/jtcc-group-e/issues/70) で実装した（[STATUS の F14 アプリ](STATUS.md)、[レビュー記録](ux-reviews/f14-app.json)）。T10 の `312:*` の4画面の重なりと番号の重複は、6段目へ移して 25–28 に直した（記録は ADOPTED_DESIGN）。

## 5. 【新規】ネイティブアプリのような操作感（方法1: 全画面表示とジェスチャーの抑制）— **これだけを進める**
**3 の案内（#57）**: マスター `448:14192`・`448:14267` に足し、街ホームだけに出す形で実装した（ルームには出さない。担当者の確認待ち）。実機の確認は担当者（MULTI_DEVICE.md の手順 0）。
