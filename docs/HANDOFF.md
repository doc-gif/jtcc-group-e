...
- 権利表記: 写真を出すときだけ、結果の舞台の下端に素材ライブラリのカードと同じ1行を出す。担当者はマスターへの追加を承認（2026-09-26「マスターに追加」）。マスターのロック（#50、F14）の間は触らず、提案を Town v2 の `task/claude/f15-pitch-photos`（`369:8216`、複製 `369:8221`）に置いた。#50 の後に `267:9379` へ同じ帯を入れる（ADOPTED_DESIGN.md の T10 の 9）。
