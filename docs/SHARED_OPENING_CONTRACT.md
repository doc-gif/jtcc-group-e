...
この契約は実装・デザインの入力であり、公開アプリの接続済み機能や実 DB の検証結果ではない。既存の端末内の模擬ルーム（1 台のブラウザのタブの間だけで動く）はそのまま残す。`src/realtime/protocol.ts` の `RoomTransport` を、ローカル専用 `MockRoomServer.asUser()` と Supabase `supabaseTransport()` の共通境界とする。どちらも `Snapshot` と下記の失敗コードを返し、UI は通信経路を黙って切り替えない。
