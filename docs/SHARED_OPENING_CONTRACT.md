...
- 1〜12 文字。空白（全角・NBSP などの Unicode の空白を含む）の続きは 1 つにし、前後の空白を除いて保存する。制御文字と見えない書式文字（ゼロ幅・双方向の上書き・BOM など）は使えない。
