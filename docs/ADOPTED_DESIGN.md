...
- T03 の [共有契約](SHARED_OPENING_CONTRACT.md)を真実の状態とする。結果は `startsAt` 前は全員非公開。公開後は保存済みの同じ結果を返す。見守り、満席復帰、予約の自動開始、二重請求防止はサーバー判定。ホストは担当者だけで、参加者によるホスト交代は廃止（ホスト用リンクでの再開に置き換え。サーバー側は F10。それまでの契約の `claim` は旧仕様）。Figma の時間遷移は説明用で、実装タイマーの根拠にしない。
...
- **配色（F06 で Web を合わせた）:** 担当者の決定でマスターは更新済み（主操作 `#be6482`＋19px 太字の白文字、小さいピンク文字はワイン `#a94a68`、タブ選択の淡いミントは残す、文字色はマスター）。F04 の画面は色・ボタンの文字サイズを直書きせず、共通のトークン（`--main`・`--deep`・`--soft`・`--text`・`--sub`・`--head`）とボタンの共通クラスだけを使うので、F06 の切り替えでまとめて揃った。小さいピンク文字は `--deep`（`#A94A68`＝ワイン）。
- **F04 の Figma 修正待ち**（Web は規則のためマスターと違う。担当者の判断後に Figma を直す）:
  1. （配色は上のとおり F06 で Web 側を合わせた。F04 の時点では主操作が `#A94A68`、タブ選択がピンク `--soft` だった。）
