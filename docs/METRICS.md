...
| デモへの到達 | GA4 はページごとにセッション数を返すので、正確な数は「下限（デモのページ別セッション数の最大値）」と「上限（合計）」の間にある。下限と上限が同じならその値が正確 |
| Clarity の平均（表示ページ数・スクロール・デッドクリック率） | URL の行を**セッション数で重み付け**して平均する |
| Clarity のプロジェクト全体 | 次元なしで取った重複のない数。LP・デモの列は URL 別の合計なので、同じ人が複数の URL に出るとユーザーが重複する（全体 < LP + デモ になることがある） |

### 判断の目安

- しっかり見た割合が低い（例: 30% 未満）→ 最初の画面で離脱している。LP の最初の画面にデモの入口があるかを見る。
- デモに進んだ割合が、しっかり見た割合より大きく下がる → 導線（ボタン・QR）の位置か文言を疑う。Clarity のデッドクリックの録画で、押されたのに反応しない場所を探す。
