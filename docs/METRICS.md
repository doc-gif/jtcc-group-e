...
- 出るもの: LP のセッション・ユーザー・**しっかり見た**（GA4 の「エンゲージのあったセッション」= 10 秒以上か 2 ページ以上）・90% スクロール・平均滞在・ページ/セッション・**一般公開アプリのページも見たセッションと割合**・一般公開アプリ側のイベント（本番公開後）。Clarity は LP とデモそれぞれのセッション（ボット除外数）・ページ/セッション・スクロール・デッドクリック・イライラしたクリック（過去 3 日、API の上限）。
