...
| 読むとき | その端末に入ったルームの記録かホスト用キーがあるときに 1 回、ルームに入ったときに読めなかった分をもう 1 回。絵はメモリの中だけに持ち、端末に保存しない。退室・期限切れでルームの記録もホスト用キーも無くなったら捨てる |
| 書き込み | アプリからはできない（読み取りのポリシーだけ）。Dashboard からだけ |
| 絵がないとき | 未アップロード・読めない・壊れた画像のときは **何も出さない**（枠や空の箱も出さない。画面の配置は変わらない）。エラーは出ない |
| E2E・端末内の模擬ルーム | 絵を読まない（何も出ないのが正しい動作） |
| 確認用プレビュー | 成果物には入らない。プレビューのビルドも本番と同じ Supabase につながるので、プレビューでルームを作った・入った人には本番と同じ条件で出る（写真の F15 と同じ扱い） |
