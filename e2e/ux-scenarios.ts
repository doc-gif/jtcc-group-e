...
export const uxScenarios: Array<{
  ...
  // #88: 開始したらすぐ回す（29）→ カプセル（30）→ 1回タップ → 当たり（31）→ みんなを待つ（32）。見守る人は 33
  { name: 'room-draw', path: roomPath, scalableText: '.spin-lead', room: roomSeed({ round: { ...revealedRound(true), revealAt: ROOM_T0 + 15_000 }, opened: false, pitchMode: true }), heading: 'ガチャを回そう' },
  { name: 'room-capsule', path: roomPath, scalableText: '.spin-lead', room: roomSeed({ round: { ...revealedRound(), revealAt: ROOM_T0 + 15_000 }, opened: false }), steps: [{ click: '1タップで1回転' }, { click: '1タップで1回転' }, { click: '1タップで1回転' }], heading: 'カプセルを開けよう' },
  { name: 'room-my-result', path: roomPath, scalableText: '.room-line', room: roomSeed({ round: { ...revealedRound(), revealAt: ROOM_T0 + 15_000 } }), heading: 'あなたの結果' },
  { name: 'room-my-result-pitch', path: roomPath, scalableText: '.room-line', room: roomSeed({ round: revealedRound(true), seen: [1], pitchMode: true }), steps: [{ click: '自分の結果をもう一度見る' }], heading: 'あなたの結果' },
  { name: 'room-wait-entrant', path: roomPath, scalableText: '.room-body', room: roomSeed({ round: { ...revealedRound(), revealAt: ROOM_T0 + 15_000 }, seen: [1] }), heading: 'みんなを待っています' },
  { name: 'room-wait-watcher', path: roomPath, scalableText: '.room-body', room: roomSeed({ round: { ...revealedRound(), results: revealedRound().results.filter((result) => result.userId !== 'me'), revealAt: ROOM_T0 + 15_000 } }), heading: 'みんなが開けています' },
  { name: 'room-results-ready', path: roomPath, scalableText: '.room-body', room: roomSeed({ round: revealedRound(), seen: [1] }), heading: 'みんなの結果が出ました' },
  { name: 'room-all-results', path: roomPath, scalableText: '.room-card-title', room: roomSeed({ round: revealedRound(), seen: [1] }), steps: [{ click: 'みんなの結果を見る' }], heading: 'みんなのピース' },
  { name: 'room-history', path: roomPath, scalableText: '.room-card-title', room: roomSeed({ round: revealedRound(), seen: [1] }), steps: [{ click: '自分の結果をもう一度見る' }, { click: '自分の履歴を見る' }], heading: '自分の履歴' },
